function openerp_pos_models(instance, module){ //module is instance.point_of_sale
    ...
    module.PosModel = Backbone.Model.extend({
        ...
        models: [
        {
            ...
            model:  'res.company',
            fields: [ 'currency_id', 'email', 'website', 'company_registry', 'vat', 'name', 'phone', 'partner_id' , 'country_id', 'tax_calculation_rounding_method','logo_web'],
    ...
    module.Orderline = Backbone.Model.extend({
        ...
        get_quantity_unit_name: function(){
            var unit = this.get_unit();
        ...
        get_quantity_in_base_unit: function(){
            var unit = this.get_unit();
            total=0;

            if(unit && !unit.is_unit){
                if(unit.uom_type=='bigger'){
                    total= this.quantity / unit.factor;
                } else if(unit.uom_type=='smaller'){
                    total= this.quantity * unit.factor;
                } else {
                    total=this.quantity;
                }
                ...
                if (unit.category_id[1]=='Peso'){
                    return  Math.ceil(total).toFixed(0) +  ' Kg';
                }
            }
            return '';
        },
